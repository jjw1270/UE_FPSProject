...
bool USignupWidget::CheckID(const FString& NewID)
{
	...
	for (const auto& Ch : NewID)
	{
		...
		if (!((Ch >= 'a' && Ch <= 'z') || (Ch >= 'A' && Ch <= 'Z') || 
			(Ch >= '0' && Ch <= '9')))
		{
			TextBlock_Info->SetText(FText::FromString(TEXT("아이디는 영문,숫자 조합만 가능합니다.")));
			return false;
		}
	}
...
bool USignupWidget::CheckUsername(const FString& NewUsername)
{
	...
	for (const auto& Ch : NewUsername)
	{
		...
		if (!((Ch >= 'a' && Ch <= 'z') || (Ch >= 'A' && Ch <= 'Z') ||
			...
			(Ch >= '0' && Ch <= '9')))
		{
			TextBlock_Info->SetText(FText::FromString(TEXT("닉네임은 영문 또는 한글, 그리고 숫자 조합만 가능합니다.")));
			return false;
		}
	...
	{
		TextBlock_Info->SetText(FText::FromString(TEXT("닉네임은 영문 4~16자 또는 한글 2~8자 내외입니다.")));
		return false;
	}
